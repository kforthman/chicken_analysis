...
for ii = 1:num_sessions
   ...
   J   = J_default + J_slope*session_data{ii}(:,4); %-kf-% (data.H(:,1)./(1-data.H(:,1)))
   Hvc = 1./(1+exp(-J));
   
   % get posteriors
   %-kf-% Calls a function written in C. Works on any Mac.
   %-kf-% q1 = (1-H)/H + exp(-L_(n-1))
   %-kf-% q2 = (1-H)/H + exp(+L_(n-1))
